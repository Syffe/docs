...
def upgrade() -> None:
    ...
    alembic.op.create_table(
        ...
        sqlalchemy.Column("oauth_access_token", sqlalchemy.String(), nullable=False),
        sqlalchemy.PrimaryKeyConstraint("id"),
    )
    alembic.op.create_table(
        "pull_request",
...
def downgrade() -> None:
    ...
    alembic.op.drop_index(
        "gha_job_run_owner_id_started_at_idx", table_name="gha_job_run"
    )
    alembic.op.drop_index(
        "gha_job_run_owner_id_repository_started_at_idx", table_name="gha_job_run"
    )
    alembic.op.drop_table("gha_job_run")
    alembic.op.drop_table("pull_request")
    alembic.op.drop_table("github_user")
    alembic.op.drop_table("github_account")
    # ### end Alembic commands ###

    alembic.op.execute("DROP TYPE jobrunconclusion;")
    alembic.op.execute("DROP TYPE jobruntriggerevent;")
