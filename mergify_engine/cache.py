...
class Cache(typing.Generic[_K, _V]):
    _cache: dict[_K, _V] = dataclasses.field(default_factory=dict)

    def get(self, key: _K) -> _V | UnsetType:
    ...
    def __getitem__(self, key: _K) -> _V | UnsetType:
        return self.get(key)
...
class SingleCache(typing.Generic[_V]):
    _cache: _V | UnsetType = dataclasses.field(default=Unset)

    def get(self) -> _V | UnsetType:
        return self._cache
