...
class CopyExecutor(actions.ActionExecutor["CopyAction", CopyExecutorConfig]):
    ...
    async def create(
        ...
        try:
            ...
            raise rules.InvalidPullRequestRule(
                f"Invalid {cls.KIND.capitalize()} title",
                str(rmf),
            )
        try:
            ...
            raise rules.InvalidPullRequestRule(
                f"Invalid {cls.KIND.capitalize()} body",
                str(rmf),
            )
        try:
        ...
        assignees = list(
            await action_utils.render_users_template(ctxt, action.config["assignees"])
        )
    ...
    async def run(self) -> check_api.Result:
        ...
        results = [await self._copy(branch_name) for branch_name in branches]
