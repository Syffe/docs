...
async def test_farthest_non_matching_schedule_datetime_filter() -> None:
    # Saturday
    with freeze_time("2022-11-12", tz_offset=0):
        tree = parser.parse("schedule=MON-FRI 08:00-17:00")
        # Next expected farthest date: Monday, 08:00:01 UTC
        assert await filter.FarthestNonMatchingScheduleDatetimeFilter(tree)(
            FakePR({"current-time": date.utcnow()})
        ) == datetime.datetime(2022, 11, 14, 8, 0, 1, tzinfo=datetime.timezone.utc)

        tree = {
            "and": [
                parser.parse("schedule=TUE-FRI 08:00-17:00"),
                parser.parse("schedule=MON-FRI 08:00-17:00"),
            ]
        }
        # Next expected farthest date: Tuesday, 08:00:01 UTC
        assert await filter.FarthestNonMatchingScheduleDatetimeFilter(tree)(
            FakePR({"current-time": date.utcnow()})
        ) == datetime.datetime(2022, 11, 15, 8, 0, 1, tzinfo=datetime.timezone.utc)
        ...
        tree = {
            "or": [
                ...
                {
                    "and": [
                        parser.parse("base=test"),
                        parser.parse("schedule=TUE-FRI 10:00-17:00"),
                    ],
                },
            ]
        }
        # Next expected farthest date: Tuesday, 10:00:01 UTC
        assert await filter.FarthestNonMatchingScheduleDatetimeFilter(tree)(
            FakePR({"base": "main", "current-time": date.utcnow(), "label": []})
        ) == datetime.datetime(2022, 11, 15, 10, 0, 1, tzinfo=datetime.timezone.utc)

        tree = {
            "and": [
                parser.parse("schedule!=SAT-MON"),  # TUE-FRI
                parser.parse("schedule=MON-FRI"),
            ]
        }
        # Next expected farthest date: Tuesday, 00:00:01 UTC
        assert await filter.FarthestNonMatchingScheduleDatetimeFilter(tree)(
            FakePR({"current-time": date.utcnow()})
        ) == datetime.datetime(2022, 11, 15, tzinfo=datetime.timezone.utc)
